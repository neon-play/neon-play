...
(function () {
    ...
    if (Array.isArray(data)) {
      ...
      if (!items.length) {
        for (const v of Object.values(data)) {
          if (v && typeof v === 'object' && (v.id || v.title || v.name)) {
            items.push(v);
          }
        }
      }
  ...
  function findMatch(items, id) {
    ...
    return items.find(it =>
      String(it.title || it.name || '').toLowerCase().includes(needle)
    );
  }
  ...
  function render(item) {
    ...
    if (item.episodes) badges.push(`${item.episodes} EP`);
    if (item.duration) badges.push(item.duration);
    if (item.views) badges.push(`${item.views} views`);
